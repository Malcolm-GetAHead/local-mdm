...
- **Retrospective scope includes ALL completed sprints.** Don't just check the current sprint's docs — scan all previous sprint OVERVIEWs for status lines and unchecked DoD boxes. Sprint 4 retrospective missed unchecked boxes in Sprint 1 and 2 because it only audited Sprint 4 files. Also: the **first** retrospective action is to update the current sprint's own OVERVIEW — mark status as complete, check all DoD boxes, update stale descriptions. This was missed in Sprint 4 until the owner caught it.
